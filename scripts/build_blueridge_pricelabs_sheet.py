"""Build PriceLabs override CSV for Blue Ridge from the manual pricing sheet.

Source: data/blueridge/Final Pricing (after multipliers).csv
Output: output/blueridge/pricing/pricelabs_override_sheet_<start>_to_<end>.csv

Listing mapping (OwnerRez / orbirental):
  3527b9f8-e4db-4220-8f47-f41ecda4d983  Little Luxe Lodge - Wellness Experience
  0dfcb2d6-226c-4a4a-a6eb-378ef134cd94  Luxe Lodge A-Frame
"""
...
DEFAULT_SOURCE = (
    ...
    / "data"
    / "blueridge"
    / "Final Pricing (after multipliers).csv"
)
DEFAULT_OUT_DIR = PROJECT_ROOT / "output" / "blueridge" / "pricing"

LISTING_MAP: dict[str, tuple[str, str]] = {
    "Little Luxe Lodge": (
        "3527b9f8-e4db-4220-8f47-f41ecda4d983",
        "Little Luxe Lodge - Wellness Experience",
    ),
    "Luxe Lodge": (
        "0dfcb2d6-226c-4a4a-a6eb-378ef134cd94",
        "Luxe Lodge A-Frame",
    ),
}
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--blank-reason",
        action="store_true",
        help="Leave Reason empty even when Holidays/Events or Notes is set.",
    )
    return p.parse_args()
...
def _reason(row: dict[str, str], blank_reason: bool) -> str:
    if blank_reason:
        return ""
    event = (row.get("Holidays/Events") or "").strip()
    notes = (row.get("Notes") or "").strip()
    return event or notes
...
def main() -> None:
    ...
    for row in rows_in:
        ...
        reason = _reason(row, args.blank_reason)
